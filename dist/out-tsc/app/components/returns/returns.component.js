...
let ReturnsComponent = class ReturnsComponent {
    returnService;
    orderService;
    invoiceService;
    authService;
    ...
    // Whole Bill Return
    showWholeReturnForm = false;
    wholeReturnReason = '';
    wholeReturnRefundMethod = 'Cash';
    wholeReturnNotes = '';
    // Partial Return
    showPartialReturnForm = false;
    partialReturnItems = [];
    partialReturnReason = '';
    partialReturnRefundMethod = 'Cash';
    partialReturnNotes = '';
    // Return Details Modal
    showDetailsModal = false;
    selectedReturn = null;
    // Invoice Products Preview Modal
    showProductsPreviewModal = false;
    previewInvoice = null;
    // Filter properties for returns list
    statusFilter = 'All';
    searchTerm = '';
    constructor(returnService, orderService, invoiceService, authService) {
    ...
    get filteredReturns() {
        ...
        if (this.searchTerm) {
            ...
            filtered = filtered.filter(r => r.customerFullName?.toLowerCase().includes(term) ||
    ...
    loadLast14DaysInvoices() {
        ...
        this.invoiceService.getAllInvoices().subscribe({
            next: (data) => {
                ...
                const orderFetchRequests = recentInvoices.map(invoice => this.orderService.getOrderById(invoice.orderId));
    ...
    extractProductsFromInvoice(invoice) {
    ...
    selectInvoiceForReturn(invoice) {
    ...
    onPartialItemSelectionChange(item) {
    ...
    calculatePartialItemAmount(item) {
    ...
    updateReturnStatus(returnItem, newStatus) {
        if (!this.isAdmin()) {
            Swal.fire({
                icon: 'warning',
                title: 'Access Denied',
